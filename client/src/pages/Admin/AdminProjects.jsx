...
function AdminProjects() {
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (selectedItemForEdit) {
        response = await axiosClient.put(`/api/portfolio/update-project`, {
          ...values,
          _id: selectedItemForEdit._id,
        });
      } else {
        response = await axiosClient.post("/api/portfolio/add-project", values);
      }
  ...
  const handleDelete = async (item) => {
    try {
      dispatch(ShowLoading());
      const response = await axiosClient.post("/api/portfolio/delete-project", {
        _id: item._id,
      });
      dispatch(HideLoading());
      if (response.data.success) {
